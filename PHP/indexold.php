...
<head>
    ...
    <title>La bière du boulanger</title>
</head>
...
// <img src="./index.jpg" alt="image_de_la_bière" position="float">
...
$beerArray = [
    ...
    for($i=0; $i<count($beerArray); $i++){
        ...
        for($j=0;$j<4;$j++){
            if($j==1){
                $beerArray[$i][$j]="<div class='image'><img src=".$beerArray[$i][$j]." width=10% /></div>";
            }
            if($j==3){
                $beerArray[$i][$j].="€";
            }
            echo($beerArray[$i][$j]."<br><br>");
        }
        if($i%3==2){
            echo('</div>');
        }
